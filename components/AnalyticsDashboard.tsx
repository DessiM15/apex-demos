...
export default function AnalyticsDashboard({ stats, packageName, basic = false }: Props) {
  // Basic mode: PulseMarket — show only core stat cards
  const displayStats = basic ? stats.slice(0, 4) : stats
  ...
  return (
    <section className="bg-apex-navy py-20 px-4">
      <div className="max-w-6xl mx-auto">
        ...
        >
          <span className={`text-xs font-bold uppercase tracking-widest px-3 py-1 rounded-pill ${
            basic
              ? 'text-apex-blue bg-apex-blue-light'
              : 'text-apex-red bg-apex-red-light'
          }`}>
            {packageName} {basic ? 'Feature' : 'Exclusive'}
          </span>
          <h2 className="text-3xl md:text-4xl font-bold text-white mt-4 mb-3">
            {basic ? 'Performance Snapshot' : 'Your Analytics Dashboard'}
          </h2>
          <p className="text-white/60 text-lg max-w-2xl mx-auto">
            {basic
              ? 'Track your key metrics at a glance — see how your landing pages and social posts perform.'
        ...
        {/* Stat Cards */}
        <motion.div
          className={`grid gap-4 mb-10 ${basic ? 'grid-cols-2 md:grid-cols-4' : 'grid-cols-2 md:grid-cols-3'}`}
        ...
        {/* Full Chart — hidden in basic mode */}
        {!basic && (
          <>
            <motion.div
